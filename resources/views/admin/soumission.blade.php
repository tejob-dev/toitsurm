    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Liste des inscriptions enrégistrées</h2>
                    </div>
                    <div class="col-sm-6">
                        <div class="d-flex justify-content-end" style="width:450px; margin-left: auto;">
                            ...
                            <a href="/manage-quiz/export/all" class="btn btn-white bg-white text-dark" style="width:230px;"><span>Exporter tous en Excel</span></a>
                        </div>
                        <div class="d-flex justify-content-end my-2" style="width: 450px; margin-left: auto;">
                            <form action="/soumission/filter" method="post" class="w-50">
                                @csrf
            ...
            @php
                ...
                foreach ($soumissions as $key => $data){
                    ...
                    for($i = 1; $i <= 18; $i++){
                        $quest = "question_".$i;
                        if($data->$quest != null || ($data->$quest == null && $i <= 16)){
                            $subm[] = $data->$quest;
                        }
                    }
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    @foreach($allsubm as $id => $data)
                        @php 
                            $len = sizeof($data);
                        @endphp
                        <tr>
                            <td colspan="2" >{{isset($data[$len-5])?$data[$len-5]." ".$data[$len-4]:"--"}}</td>
                            <td >{{isset($data[$len-6])?$data[$len-6]:"--"}}</td>
                            <td >{{isset($data[$len-3])?$data[$len-3]:"--"}}</td>
                            <td >{{isset($data[$len-2])?$data[$len-2]:"--"}}</td>
                            <td >{{isset($data[$len-1])?$data[$len-1]:"--"}}</td>
                            <td>
                                <a href="{{'/manage-quiz/'.$id.'/edit'}}" class="edit"><i class="material-icons" data-toggle="tooltip" title="Edit les détails">&#xE3C9;</i></a>
                                <a href="{{'/manage-quiz/'.$id}}" class="view"><i class="material-icons" data-toggle="tooltip" title="Voir les détails">&#xE417;</i></a>
                                <a href="#deleteEmployeeModal" onclick="document.getElementById('hideitemid').value = {{$id}}; document.getElementById('typeproc').value = 1;" class="delete" data-toggle="modal" ><i class="material-icons" data-toggle="tooltip" title="Supprimer cet élément">&#xE872;</i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            <div class="flex items-center mt-2">
                ...
                <p style="font-size: 18px;">
                    Affichage {{$soumissions->count()}} sur {{ $soumissions->total() }} questionnaire(s).
                </p>
            </div>
        </div>
    </div>
    <div id="deleteEmployeeModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="/manage-soumiss/suppr" >
                    @csrf

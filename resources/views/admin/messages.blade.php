    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Liste des personnes nous ayant écrit</h2>
                    </div>
                    <div class="col-sm-6">
                        <div class="d-flex justify-content-end" style="width:450px; margin-left: auto;">
                            ...
                            <a href="/manage-contact/export/all" class="btn btn-white bg-white text-dark" style="width:230px;"><span>Exporter en Excel</span></a>
                            <!--Okay-->
                        </div>
                        <div class="d-flex justify-content-end my-2" style="width: 450px; margin-left: auto;">
                            <form action="/contacts/filter" method="post" class="w-50">
                                @csrf
                ...
                <tbody>
                    @foreach ($contacts as $key => $data)
                        ...
                        <tr>
                            ...
                            <!--    </span>-->
                            <!--</td>-->
                            <td colspan="2">{{isset($data->nom)?$data->nom:""}}<span>&nbsp;</span>{{isset($data->nom)?$data->prenom:"Pas de donnée"}}</td>
                            <td colspan="1">{{isset($data->telephone)?$data->telephone:"Pas de donnée"}}</td>
                            <td colspan="1">{{isset($data->email)?$data->email:"Pas de donnée"}}</td>
                            <td colspan="3">{{isset($data->message)?$data->message:"Pas de donnée"}}</td>
                            {{-- <td>{{date('d/m/Y', strtotime($data->created_at))}}</td> --}}
                            <td>
                                {{-- <a href="{{"/manage-quiz/".$data->id}}" class="view"><i class="material-icons" data-toggle="tooltip" title="Voir les détails">&#xE417;</i></a> --}}
                                <a href="{{'/manage-contact/'.$data->id.'/edit'}}" class="edit"><i class="material-icons" data-toggle="tooltip" title="Edit les détails">&#xE3C9;</i></a>
            ...
            <div class="flex items-center mt-2">
                ...
                <p style="font-size: 18px;">
                    Affichage {{$contacts->count()}} sur {{ $contacts->total() }} message(s).
                </p>
            </div>
        </div>
    </div>
    <div id="deleteEmployeeModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="/manage-contact/delete" >
                    @csrf
